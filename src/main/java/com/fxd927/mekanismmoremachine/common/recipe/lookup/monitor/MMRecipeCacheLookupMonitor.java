package com.fxd927.mekanismmoremachine.common.recipe.lookup.monitor;
...
public class MMRecipeCacheLookupMonitor<RECIPE extends MekanismRecipe> implements ICachedRecipeHolder<RECIPE>, IContentsListener {
    private final IMMRecipeLookupHandler<RECIPE> handler;
    protected final int cacheIndex;
    protected CachedRecipe<RECIPE> cachedRecipe;
    protected boolean hasNoRecipe;

    public MMRecipeCacheLookupMonitor(IMMRecipeLookupHandler<RECIPE> handler) {
        this(handler, 0);
    }

    public MMRecipeCacheLookupMonitor(IMMRecipeLookupHandler<RECIPE> handler, int cacheIndex) {
